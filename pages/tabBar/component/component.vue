<template>
	<view class="main">
		<!-- #ifdef APP-PLUS -->
		<!-- 固定在顶部的导航栏 -->
		<uni-nav-bar fixed="true" left-icon="location" @click-left="chooseLocation" @click-right="scanCode" :left-text="locationAddress" right-icon="scan" title="驾图预想"></uni-nav-bar>
		<!-- 占位用的导航栏 -->
		<uni-nav-bar background-color="rgba(0,0,0,0)" status-bar="true"></uni-nav-bar>
		<!-- #endif -->
		<view  class="wrapper">
			<swiper class="my_swiper" :indicator-dots="indicatorDots" :autoplay="autoplay" :circular="true" :duration="duration">
				<!-- 添加这个属性:circular="true"   轮播图更流畅，到最后一张图时，继续滑动会顺向滑动到第一张 -->
		...
		<view class="info">
			...
			<view class="info_gb"> 
				...
				<swiper class="neir" vertical="true" autoplay="true" circular="true" interval="3000">
					<swiper-item>
						<navigator>赶紧行动，成为车联网车主！！！</navigator>
					</swiper-item>
					<swiper-item>
						<navigator>活动最新消息！！！</navigator>
					</swiper-item>
					<swiper-item>
						<navigator>你敢买，我敢送！！！</navigator>
					</swiper-item>
				</swiper>
			</view>
		</view>
			...
			<view class="page-section">
				<view class="list-view content-section">
					<view class="item list-item" @tap="clickItem(index)" v-for="(item,index) in items" :key="index">
						...
						<view class="sum_buy">
							<view class="money">
								<text class="money_je">¥1368</text><text class="yuan">元</text>
							</view>
...
<script>
	...
	export default { 
		...
		computed: mapState(['hasLogin','isLogin', 'username','isGesturePW']), // 获取Vuex仓库里面的状态值
		onLoad() {
		...
		methods: {
			...
			clickItem(index){ // 点击功能菜单
				...
				}else if(!this.hasLogin && this.isGesturePW == true){
					// 如果有账号(缓存记录)，但是没有登录，则显示手势登录页面
					if(this.username){
						uni.navigateTo({
